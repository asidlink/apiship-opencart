...
class Apiship extends \Opencart\System\Engine\Controller { 
	...
	public function index() {  
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('extension/apiship/shipping/apiship', 'user_token=' . $this->session->data['user_token'] . '&type=shipping'));
		...
		$data['shipping_apiship_version'] = '1.2 (OpenCart 4.1.0.3)';
		...
		$data['url_callback'] = $server . 'index.php?route=extension/apiship/shipping/apiship/callback';
		$data['shipping_apiship_search_point_url'] = $server . 'index.php?route=extension/apiship/shipping/apiship.get_point';
		...
		if (isset($this->request->post['shipping_apiship_crate_cost'])) {
			$data['shipping_apiship_crate_cost'] = $this->request->post['shipping_apiship_crate_cost'];
		} else { 
			$data['shipping_apiship_crate_cost'] = $this->config->get('shipping_apiship_crate_cost');
		} 

		if (isset($this->request->post['shipping_apiship_crate_dimension_add'])) {
			$data['shipping_apiship_crate_dimension_add'] = $this->request->post['shipping_apiship_crate_dimension_add'];
		} else { 
			$data['shipping_apiship_crate_dimension_add'] = $this->config->get('shipping_apiship_crate_dimension_add');
		} 

		if (isset($this->request->post['shipping_apiship_crate_weight_add'])) {
			$data['shipping_apiship_crate_weight_add'] = $this->request->post['shipping_apiship_crate_weight_add'];
		} else { 
			$data['shipping_apiship_crate_weight_add'] = $this->config->get('shipping_apiship_crate_weight_add');
		} 
		...
		$data['action'] = $this->url->link('extension/apiship/shipping/apiship', 'user_token=' . $this->session->data['user_token']);
		
		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=shipping');

		$catalog_server = ((!empty($this->request->server['HTTPS'])) ? $this->config->get('config_ssl') : $this->config->get('config_url'));

		// В админке настройки config_url/config_ssl могут быть пустыми — берём URL витрины.
		if (empty($catalog_server)) {
			if (defined('HTTP_CATALOG')) {
				$catalog_server = HTTP_CATALOG;
			} else {
				$catalog_server = HTTP_SERVER;
			}
		}

		$data['shipping_apiship_import_cron_url'] = $catalog_server . "index.php?route=extension/apiship/shipping/apiship/import_orders";
		$data['shipping_apiship_export_cron_url'] = $catalog_server . "index.php?route=extension/apiship/shipping/apiship/export_orders";
	...
	/**
	 * ApiShip order management tab.
	 *
	 * Метод вызывается со страницы заказа админки (sale/order.info) через
	 * $this->load->controller('extension/apiship/shipping/apiship.order'),
	 * поэтому возвращает готовый HTML. Возвращаемый вывод также кладётся в
	 * response, чтобы работал и прямой вызов маршрута.
	 *
	 * @return string HTML вкладки (или пустая строка при отсутствии прав)
	 */
	public function order(): string {
		...
		// В админке настройки config_url/config_ssl могут быть пустыми
		// (в oc_setting их нет) — тогда берём базовый URL витрины.
		if (empty($catalog_server)) {
			if (defined('HTTP_CATALOG')) {
				$catalog_server = HTTP_CATALOG;
			} else {
		...
		// Телефон получателя для вкладки ApiShip: из заказа, при пустом — из карточки покупателя.
		$recipient_phone = '';
		...
		if ($order_info) {
			$recipient_phone = $order_info['telephone'] ?? '';

			if (empty($recipient_phone) && !empty($order_info['customer_id'])) {
				$customer_query = $this->db->query("SELECT `telephone` FROM `" . DB_PREFIX . "customer` WHERE `customer_id` = '" . (int)$order_info['customer_id'] . "'");
		...
		$data['shipping_apiship_recipient_phone'] = $recipient_phone;

		$data['shipping_apiship_export_url'] = $catalog_server . 'index.php?route=extension/apiship/shipping/apiship.export_order&id=' . $order_id . '&key=' . $this->config->get('shipping_apiship_cron_key');
		$data['shipping_apiship_export_cancel_url'] = $catalog_server . 'index.php?route=extension/apiship/shipping/apiship.cancel_order&id=' . $order_id . '&key=' . $this->config->get('shipping_apiship_cron_key');
		$data['shipping_apiship_get_order_params_url'] = $catalog_server . 'index.php?route=extension/apiship/shipping/apiship.get_order_params&id=' . $order_id;
		$data['shipping_apiship_label_url'] = $catalog_server . 'index.php?route=extension/apiship/shipping/apiship.get_label';
		$data['shipping_apiship_waybill_url'] = $catalog_server . 'index.php?route=extension/apiship/shipping/apiship.get_waybill';

		$keys = [
			'text_apiship',
			'entry_shipping_apiship_pickup_type',
			'entry_shipping_apiship_pickup_type1',
			'entry_shipping_apiship_pickup_type2',
			'entry_shipping_apiship_place_dimensions',
			'entry_shipping_apiship_place_weight',
			'entry_shipping_apiship_shipping_date',
			'entry_shipping_apiship_recipient_phone',
			'entry_shipping_apiship_comment',
			'entry_shipping_apiship_order_status',
			'entry_shipping_apiship_tracking_url',
			'entry_shipping_apiship_tracking_url_not_found',
			'entry_shipping_apiship_order_status_not_export',
			'entry_shipping_apiship_control',
			'entry_shipping_apiship_export_title',
			'entry_shipping_apiship_cancel_export_title',
			'entry_shipping_apiship_label_title',
			'entry_shipping_apiship_waybill_title',
			'entry_shipping_apiship_cancel_confirm',
			'entry_shipping_apiship_paid',
			'entry_shipping_apiship_crate_note',
			'error_shipping_apiship_export_network'
		];
		...
		$output = $this->load->view('extension/apiship/shipping/order', $data);
