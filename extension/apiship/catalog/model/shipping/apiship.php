...
class Apiship extends \Opencart\System\Engine\Model {    
	...
	public function __construct($params) {
		...
		$this->apiship_params = [
			...
			'shipping_apiship_crate_cost' => $this->config->get('shipping_apiship_crate_cost'),
			'shipping_apiship_crate_dimension_add' => $this->config->get('shipping_apiship_crate_dimension_add'),
			'shipping_apiship_crate_dimension_add_2' => $this->config->get('shipping_apiship_crate_dimension_add_2'),
			'shipping_apiship_crate_dimension_add_3' => $this->config->get('shipping_apiship_crate_dimension_add_3'),
			'shipping_apiship_crate_weight_add' => $this->config->get('shipping_apiship_crate_weight_add'),
			...
			'shipping_apiship_error_no_point' => $this->language->get('shipping_apiship_error_no_point'),
		...
		if (!is_array($this->apiship_params['shipping_apiship_paid_orders'])) $this->apiship_params['shipping_apiship_paid_orders'] = [];
		if (!is_array($this->apiship_params['shipping_apiship_cash_on_delivery_payment_methods'])) $this->apiship_params['shipping_apiship_cash_on_delivery_payment_methods'] = [];
	...
	private function getCartTotal() {
		...
		if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
			...
			foreach ($results as $result) {
				if ($this->config->get('total_' . $result['code'] . '_status')) {
					...
					$this->load->model('extension/' . $extension . '/total/' . $result['code']);

					// OpenCart 4 requires the callback to be invoked explicitly so references are preserved.
					($this->{'model_extension_' . $extension . '_total_' . $result['code']}->getTotal)($totals, $taxes, $total);
				}
			}
		...
		foreach ($totals as $total) {
			// «Обрешётка» — отдельная строка, в оценочную стоимость товаров не входит.
			if (($total['code']!='total')&&($total['code']!='shipping')&&($total['code']!='crate')) $end_total = $end_total + $total['value'];
	...
	private function get_image_ref($provider) {
		if ($provider == '')
			return '<img class="apiship_img_providers" src="' . ((defined('HTTP_CATALOG')) ? HTTP_CATALOG : HTTP_SERVER) . 'catalog/view/theme/default/image/shipping/apiship_map.png">';
		else
	...
	/**
	 * Название метода в списке доставок: «Доставка до ПВЗ <a>…ПВЗ…</a>».
	 * Сам пункт выдачи становится ссылкой для смены (вместо отдельной
	 * ссылки «сменить ПВЗ»).
	 */
	private function point_name_with_link(string $title, string $point_name, string $code): string {
		if ($point_name === '' || strpos($title, $point_name) === false) {
			return $title; // ПВЗ не найден в названии — оставляем название как есть
		}

		$link = '<a class="apiship_points" href="#" onclick="apiship_open(\'' . $code . '\');return false;">' . htmlspecialchars($point_name, ENT_QUOTES, 'UTF-8') . '</a>';

		return str_replace($point_name, $link, $title);
	}
  	...
  	public function get_quote_list($address, $full_list = false) {
		...
		// URL каталога: в админке HTTP_SERVER указывает на админ-папку,
		// поэтому для ассетов модуля используем HTTP_CATALOG.
		$shipping_apiship_catalog_url = (defined('HTTP_CATALOG')) ? HTTP_CATALOG : HTTP_SERVER;
...
*/
			if ($this->apiship_params['shipping_apiship_group_points']) {
					...
					if ('apiship.' . $element['key'] == $shipping_apiship_last_select_code)  {
						...
						if ($parce_code['point_id'] != 'error') {
							...
							$title = $this->fill_template([
								'template' => $this->apiship_params['shipping_apiship_title_point_template'], 
								'type' => 'point',
							'providerKey' => $parce_code['provider'],
							'tariffName' => $element['tariffName'],
				...
				if (empty($quote_data)) {
					foreach($start_points as $provider_key => $element) {
						...
						$image = $shipping_apiship_catalog_url . "extension/apiship/catalog/view/theme/default/image/shipping/apiship_map.png";
			...
			if ($points_data['error'] == 'no_error') {
				...
				foreach($points_data['points'] as $point) {										
					...
					$quote_data[$parce_code['short_code']] = [
						...
						'tax_class_id' => $this->apiship_params['shipping_apiship_tax_class_id'],
						'text'         => $point['text'],
						// Данные для карты ПВЗ в админке
						'lat'          => $point['lat'],
						'lng'          => $point['lon'],
						'address'      => $point['address'],
						'type'         => $point['type'],
						'provider'     => $point['provider'],
						'provider_key' => $point['provider_key'],
						'tariff'       => $point['tariff'],
		...
		// Keep only the cheapest courier (door) tariff so the customer sees one clear "Доставка курьером" option
		$min_door_key = null;
		$min_door_cost = null;

		foreach ($quote_data as $key => $q) {
			if (strpos((string)$key, 'door_') === 0) {
				if ($min_door_cost === null || (float)$q['cost'] < $min_door_cost) {
					$min_door_cost = (float)$q['cost'];
					$min_door_key = $key;
				}
			}
		}

		if ($min_door_key !== null) {
			foreach ($quote_data as $key => $q) {
				if (strpos((string)$key, 'door_') === 0 && $key !== $min_door_key) {
		...
		// В админке HTTP_SERVER указывает на админ-папку, поэтому для
		// ассетов карты используем URL каталога (HTTP_CATALOG).
		$shipping_apiship_catalog_url = (defined('HTTP_CATALOG')) ? HTTP_CATALOG : HTTP_SERVER;
		$shipping_apiship_css_url = $shipping_apiship_catalog_url . 'extension/apiship/catalog/view/javascript/apiship.css';
		$shipping_apiship_js_url = $shipping_apiship_catalog_url . 'extension/apiship/catalog/view/javascript/apiship.js';
		$shipping_apiship_get_last_tracing_id_url = $this->url->link('extension/apiship/shipping/apiship.get_last_tracing_id', '', 'SSL');
		$shipping_apiship_get_points_url = $this->url->link('extension/apiship/shipping/apiship.get_points', '', 'SSL');
		$shipping_apiship_set_point_url = $this->url->link('extension/apiship/shipping/apiship.set_point', '', 'SSL');
...
$apiship_script = <<<EOT
				...
				<script type="text/javascript">
					...
					function create_shipping_method_template(data) {
					  ...
					  const apishipLink = data.apiship_link
					    ? `<a class="\${data.apiship_link_class || ''}" href="#" onclick="\${data.apiship_link}">
					         ...
					         <span id="apiship_loading_\${data.code}" class="apiship_loading" style="visibility:hidden;"></span>
					       </a>`
					    : '';
					  ...
					  return `
					    <div class="form-check">
					      <input type="radio" checked="checked" value="\${data.code}" name="shipping_method" class="form-check-input" id="input-shipping-method-\${data.code}">
					      <label for="input-shipping-method-\${data.code}" class="form-check-label">
					        <span class="apiship-option">
					          <span class="apiship-option-title">\${apishipImage}\${data.title} \${apishipLink}</span>
					          <span class="apiship-option-price">\${data.text}</span>
					        </span>
					      </label>
					    </div>
					  `;
					...
					$(document).ready(function(){
						...
						for(var i=0; i<document.querySelectorAll(el).length; i++) {
							var row = document.querySelectorAll(el)[i].closest('.form-check');
							var price = row ? row.querySelector('.apiship-option-price') : null;
							if (price) {
								price.textContent = '$shipping_apiship_title_from ' + price.textContent;
							}
						}
							...
							success: function(data) {								
								...
								{	
									if ($standart_checkout)
									{
										...
										// Сразу сохраняем способ доставки после выбора ПВЗ,
										// чтобы стоимость и выбранный пункт обновились в заказе.
										$('#form-shipping-method').trigger('submit');
									}
									else
									{
				...
				<link rel="stylesheet" href="$shipping_apiship_css_url?v=$shipping_apiship_version_js_mod" type="text/css" />

				<script type="text/javascript" src="$shipping_apiship_js_url?v=$shipping_apiship_version_js_mod"></script>

EOT;

		// Обрешётка теперь показывается ОТДЕЛЬНОЙ строкой в итогах заказа
		// (расширение total «crate»), а не добавляется к тарифу доставки:
		// иначе при смене ПВЗ сумма терялась, а покупателю было непонятно,

		// Пост-обработка тарифов «Доставка до ПВЗ»:
		// 1) логотип провайдера (СДЭК и т.п.) показывается сразу, даже если ПВЗ ещё не выбран;
		// 2) выбранный ПВЗ становится ССЫЛКОЙ для смены (вместо отдельной ссылки «сменить ПВЗ»).
		foreach ($quote_data as $key => &$quote) {
			if (isset($quote['code']) && strpos($quote['code'], 'apiship.point_') === 0) {
				$parce_code = $this->apiship->parce_code($quote['code']);

				if ($parce_code['point_id'] != 'error') {
					$point = $this->apiship_point($parce_code['point_id']);

					if ($point && !empty($point['name'])) {
						$wrapped = $this->point_name_with_link((string)$quote['name'], (string)$point['name'], (string)$quote['code']);

						if ($wrapped !== $quote['name']) {
							$quote['name'] = $wrapped;

							// ПВЗ стал ссылкой — отдельную ссылку «сменить ПВЗ» убираем.
							$quote['apiship_link'] = '';
							$quote['apiship_link_class'] = '';
							$quote['apiship_link_text'] = '';
						}
					}

					// Логотип провайдера (СДЭК) показываем и после выбора ПВЗ.
					if ($this->apiship_params['shipping_apiship_icon_show'] && !empty($point['providerKey'])) {
						$quote['apiship_image'] = "https://storage.apiship.ru/icons/providers/svg/" . $point['providerKey'] . ".svg";
						$quote['apiship_image_class'] = 'apiship_img_providers';
					}
				} else {
					// ПВЗ не выбран — показываем логотип провайдера и ссылку «выбрать ПВЗ».
					if ($this->apiship_params['shipping_apiship_icon_show'] && !empty($parce_code['provider'])) {
						$quote['apiship_image'] = "https://storage.apiship.ru/icons/providers/svg/" . $parce_code['provider'] . ".svg";
						$quote['apiship_image_class'] = 'apiship_img_providers';
					}
				}
			}
		...
		// из чего складывается цена. Стоимость доставки = базовый тариф.
		...
		if(!empty($quote_data)) 
      	$method_data = array(
	      	...
	      	'name'       => $this->apiship_params['shipping_apiship_title'], 
	      	'quote'      => $quote_data,
				...
				foreach($tariff['pointIds'] as $point_id) {
					foreach(array(1,2) as $pickup_type) {
						...
						$all_points[] = [
							...
							'cost' => $this->currency->format($this->tax->calculate($this->currency->convert($cost, $this->apiship_params['shipping_apiship_rub_select'], $this->config->get('config_currency')), $this->apiship_params['shipping_apiship_tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency'], 0.0, false),						
	...
	public function export_order() {
		...
		// Экспорт с вызовом API СДЭК может длиться дольше 30 секунд
		// (max_execution_time по умолчанию) — иначе PHP «убивает» скрипт
		// после создания заказа, и браузер не получает ответ.
		set_time_limit(120);
		...
		if (isset($this->request->get['shipping_apiship_recipient_phone'])) $shipping_apiship_recipient_phone = $this->request->get['shipping_apiship_recipient_phone']; else $shipping_apiship_recipient_phone = '';
		...
		return $this->export_order_value($order_id, $shipping_apiship_pickup_type, $shipping_apiship_place_length, 
			...
			$this->apiship_params['shipping_apiship_export_status'], '', $shipping_apiship_recipient_phone);
	}
	...
	private function export_order_value($order_id, $shipping_apiship_pickup_type, $shipping_apiship_place_length, $shipping_apiship_place_width, 
		...
		$shipping_apiship_export_status_error, $shipping_apiship_recipient_phone = '') {
		...
		// OC4.1: код способа доставки лежит в массиве shipping_method['code']
		$shipping_code = (isset($order['shipping_method']['code'])) ? $order['shipping_method']['code'] : '';
		...
		{
			// ПВЗ должен быть конкретным. Если покупатель не выбрал ПВЗ на карте,
			// в коде способа доставки будет заглушка 'error' (apiship.point_…_error_…):
			// такой заказ в СДЭК отправить нельзя — ApiShip отклоняет pointOutId.
			if ($point_id === '' || $point_id === 'error' || (string)(int)$point_id !== (string)$point_id) {
				$text = $this->apiship_params['shipping_apiship_error_no_point'];
		...
		$order_params['orderPointOutId'] = ($point_id === '') ? '' : (int)$point_id;
		...
		// Телефон получателя: приоритет — введённый во вкладке ApiShip в админке,
		// затем телефон из заказа, затем телефон покупателя из oc_customer
		// (его могли заполнить позже в админке: Покупатели → карточка покупателя → Телефон).
		$recipient_phone = trim((string)$shipping_apiship_recipient_phone);

		if (empty($recipient_phone)) {
			$recipient_phone = $order['telephone'];
		}

		if (empty($recipient_phone) && !empty($order['customer_id'])) {
			$customer_query = $this->db->query("SELECT `telephone` FROM `" . DB_PREFIX . "customer` WHERE `customer_id` = '" . (int)$order['customer_id'] . "'");

			if ($customer_query->num_rows && !empty($customer_query->row['telephone'])) {
				$recipient_phone = $customer_query->row['telephone'];
			}
		}

		$order_params['recipientPhone'] =  $recipient_phone;  
		...
		if (isset($output_data['orderId'])) {
			...
			$this->model_checkout_order->addHistory($order['order_id'], $shipping_apiship_export_status_ok, $text, FALSE);
			...
			if ($shipping_apiship_export_status_error!='')
				$this->model_checkout_order->addHistory($order['order_id'], $shipping_apiship_export_status_error, $text, FALSE);
	...
	public function cancel_order() {
		...
		if (isset($output_data['orderId'])) {
			...
			$this->model_checkout_order->addHistory($order_id, $this->apiship_params['shipping_apiship_cancel_export_status'], $text, FALSE);
	...
	public function export_orders() {
		...
		// Групповой экспорт (cron) может обрабатывать много заказов —
		// снимаем лимит времени исполнения.
		set_time_limit(300);
	...
	private function getOrderProducts($order_id) {
		...
		foreach($order_products as &$order_product) {
			...
			// Обрешётка: переносим флаг с карточки товара в заказ, чтобы при
			// создании заказа в ApiShip передать службе доставки услугу PACKAGE_CRATE.
			$order_product['crate'] = (isset($product_info['crate'])) ? (int)$product_info['crate'] : 0;
		}
	...
	private function get_order_params_value($order_id) {
		...
		$parce_code = $this->apiship->parce_code(isset($order_info['shipping_method']['code']) ? $order_info['shipping_method']['code'] : '');
		...
		return [
			...
			'crate' => (int)$calculate_data['crate'],
	...
	private function get_order_totals($order_id) {
		...
		// OC4.1: итоги заказа из модели checkout/order (getTotals)
		$this->load->model('checkout/order');
		...
		foreach ($totals as $total_item) {
			// «Обрешётка» — отдельная строка, в оценочную стоимость товаров не входит.
			if (($total_item['code']!='total')&&($total_item['code']!='shipping')&&($total_item['code']!='crate')) $total_sum = $total_sum + $total_item['value'];
	...
	public function get_delivery_cost_original() {
		...
		$cash_on_delivery = in_array(isset($order_info['payment_method']['code']) ? $order_info['payment_method']['code'] : '', $this->apiship_params['shipping_apiship_cash_on_delivery_payment_methods']);
		...
		foreach($providers as $provider) {	
			...
			foreach($tariffs as $tariff) {
				foreach($tariff['pointIds'] as $point_id) {
					...
					if ($key == (isset($order_info['shipping_method']['code']) ? $order_info['shipping_method']['code'] : '')) {
	...
	public function get_apaiship_orders() {
		...
		$sql = "SELECT order_id FROM `" . DB_PREFIX . "order` where shipping_method LIKE '%apiship%' and order_status_id = " . (int)$this->apiship_params['shipping_apiship_group_export_status_ready'];
