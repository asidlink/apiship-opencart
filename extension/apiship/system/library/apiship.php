...
class Apiship {
	...
	public function getData($key) {
 	...
 	public function apiship_calculator($country, $region, $city, $postcode, $ext_address, $providers, $products, $total, $cash_on_delivery) {
		...
		// Обрешётка: услугу «Упаковка в обрешётку» (PACKAGE_CRATE) СДЭК НЕ
		// принимает по API — это подтверждено поддержкой СДЭК и ApiShip
		// (в каталоге /lists/services для cdek услуги обрешётки нет, а поле
		// services у places в API ApiShip отсутствует). Передавать её нельзя.
		// Габариты обрешётки уже учтены в размерах места (calculate_places:
		// +shipping_apiship_crate_dimension_add см по каждой стороне), поэтому
		// калькулятор корректно считает доставку с учётом обрешётки. Саму
		// услугу в СДЭК нужно добавить вручную в ЛК СДЭК / ЭК5 после
		// регистрации заказа.
		...
		// Тарифы «Сборный груз» (LTL) исключаем полностью: у отправителя в
		// Йошкар-Оле нет LTL-офиса приёма груза, и СДЭК отклоняет такие заказы
		// («Для заказов с доп. типом "сборный груз" должен быть указан ПВЗ
		// отправителя/получателя с признаком LTL»). Крупногабарит уходит
		// обычными тарифами (например «Магистральный экспресс склад-склад»).
		if (isset($data['body']['deliveryToPoint'])) {
			$data['body']['deliveryToPoint'] = $this->filter_ltl_tariffs($data['body']['deliveryToPoint']);
		}
		if (isset($data['body']['deliveryToDoor'])) {
			$data['body']['deliveryToDoor'] = $this->filter_ltl_tariffs($data['body']['deliveryToDoor']);
		}
 	...
 	public function apiship_order($order_params) {
		...
		foreach($order_params['items'] as $item) {
			...
			// При наложенном платеже стоимость единицы товара — в копейках.
			// При предоплате (codCost=0) — 0: сумма товарных позиций должна
			// совпадать с суммой к получению (codCost), иначе ApiShip отклоняет.
			$cost = $this->format_cost(($order_params['costCodCost']==0) ? 0 : $item['cost'] * $koef * 100);
			...
			$params['places'][0]['items'][] = [
				...
				'height' => $height,
				'cost' => $cost,
				// При предоплате 0 (сумма позиций должна совпадать с codCost).
				'assessedCost' => ($order_params['costCodCost']==0) ? 0 : round($item['assessed_cost'] * 100)
			];

		}

		// Обрешётка: раньше здесь в место добавлялась услуга
		// places[0].services = ['PACKAGE_CRATE']. Поле services в модели Place
		// API ApiShip отсутствует (ApiShip игнорирует его), а сама услуга
		// «Упаковка в обрешётку» СДЭК по API не передаётся (подтверждено
		// поддержкой СДЭК). Габариты обрешётки уже учтены в размерах места
		// (см. calculate_places), поэтому в СДЭК уходит корректное «место».
		// Услугу «Деревянная обрешётка» нужно добавить вручную в ЛК СДЭК /
		// ЭК5 после регистрации заказа.

		// Разница между стоимостью товаров, НП и доставкой.
		// (bcadd/bcsub убраны: расширение php-bcmath не установлено на сервере)
		...
		// Корректировка стоимости товаров применима ТОЛЬКО при наложенном платеже.
		// При предоплате (codCost=0) стоимость товаров передаём целиком в копейках,
		// иначе она становится отрицательной и ApiShip отклоняет заказ.
		if ($order_params['costCodCost'] != 0) {
	...
	public function calculate_places($products, $total_sum) {
		...
		foreach ($products as $product) {
			...
			$items[] = [
				...
				'width' => $width,
				'cost' => $cost,
				'crate' => (isset($product['crate']) ? (int)$product['crate'] : 0)
			];

			if (!empty($product['crate'])) $crate = 1;
		...
		// Обрешётка: габариты под реальные СДЭК (+5/+10/+5 к отсортированным
		// сторонам места: наибольшая/средняя/наименьшая). Для шкафа-сушилки
		// 80×60×37 это даёт обрешётку 42×85×70 — как в ЛК СДЭК (вес к оплате
		// 49,98 кг). Если настройки-стороны не заданы — используется общая
		// добавка (shipping_apiship_crate_dimension_add) ко всем сторонам.
		if ($crate) {
			$crate_add_max = (int)$this->apiship_params['shipping_apiship_crate_dimension_add'];
			$crate_add_mid = (int)$this->apiship_params['shipping_apiship_crate_dimension_add_2'];
			$crate_add_min = (int)$this->apiship_params['shipping_apiship_crate_dimension_add_3'];

			if (!$crate_add_mid) $crate_add_mid = $crate_add_max;
			if (!$crate_add_min) $crate_add_min = $crate_add_max;

			$total_length = $total_length + $crate_add_max;
			$total_width = $total_width + $crate_add_mid;
			$total_height = $total_height + $crate_add_min;
			$total_weight = $total_weight + (int)round((float)$this->apiship_params['shipping_apiship_crate_weight_add'] * 1000);
		}
	...
	/**
	 * Исключает тарифы «Сборный груз» (LTL) из ответа калькулятора.
	 *
	 * LTL-заказы требуют ПВЗ отправителя и получателя с признаком «LTL»,
	 * а у магазина в Йошкар-Оле такого офиса нет, поэтому СДЭК отклоняет
	 * такие заказы. Тарифы отфильтровываются по названию («Сборный груз»).
	 *
	 * @param array $providers ответ калькулятора (deliveryToPoint / deliveryToDoor)
	 * @return array
	 */
	private function filter_ltl_tariffs($providers) {
		$result = [];

		foreach ((array)$providers as $provider) {
			if (isset($provider['tariffs']) && is_array($provider['tariffs'])) {
				$tariffs = [];

				foreach ($provider['tariffs'] as $tariff) {
					$name = isset($tariff['tariffName']) ? mb_strtolower((string)$tariff['tariffName']) : '';

					if (mb_strpos($name, 'сборный груз') !== false) {
						continue;
					}
